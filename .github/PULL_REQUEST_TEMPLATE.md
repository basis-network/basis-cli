## What breaks today

<!-- What does not work, or what is wrong, before this change. -->

## What this changes

<!-- And why this way. Comments and descriptions explain WHY, not what. -->

## Checklist

- [ ] `shellcheck download.sh` passes
- [ ] New files carry an SPDX header, or an entry in `REUSE.toml`
- [ ] Commits are signed off (`git commit -s`) — see [CONTRIBUTING.md](../CONTRIBUTING.md)
- [ ] If this changes what the repository vouches for (checksums, what is
      distributed, how it is verified), it is in `CHANGELOG.md` with the reasoning
